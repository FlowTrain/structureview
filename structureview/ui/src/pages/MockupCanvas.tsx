...
function loadInitial(): MockupDoc {
  try {
    if (typeof localStorage !== 'undefined') {
      const raw = localStorage.getItem(STORAGE_KEY)
      if (raw) {
        const parsed = JSON.parse(raw)
        if (parsed && Array.isArray(parsed.regions)) return normalise(parsed)
      }
    }
  } catch {
    // corrupt or unavailable — fall through to an empty canvas
  }
  return emptyDoc()
}
...
export function MockupCanvas() {
  const [doc, setDoc] = useState<MockupDoc>(loadInitial)
  ...
  return (
    <div className="mc-shell">
      ...
      <div className="mc-body">
        ...
        {/* RIGHT: inspector — or, on Reveal, the JSON tree */}
