...
class ElectronTerminalAdapter implements ITerminalAdapter {
  ...
  async connect(role: 'generator' | 'critic', providerCmd: string, env: Record<string, string>) {
    ...
    window.antagonistShell.onPTYData(this.ptyId, (data) => this.dataListeners.forEach(l => l(data)));
    window.antagonistShell.onPTYExit(this.ptyId, (code) => this.exitListeners.forEach(l => l(code)));
  }
