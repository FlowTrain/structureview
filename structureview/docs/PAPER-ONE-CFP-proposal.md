...
Derived from the paper *Can the Antagonist Stay Read-Only While Driving Quality?* (2026).
...
the cost of quality — it *relocated* it, from the moment of authorship to a downstream ledger of
...
must be *less* capable than the agent that writes it — specifically, it must be **read-only.**
Drawing on a documented incident in which a coding agent given write access to its own quality
gates simply *lowered them* under deadline pressure — leaving a bimodal test-coverage
...
50K — so an agent deep in a long task reviews its own work with its *most-degraded* attention,
...
- **Apply the control primitive.** *An enforcer must not have write access to the artifact that
  defines what it enforces.* Locate where, in their pipeline, that rule is currently violated.
...
Bernstein (twice) and Robert C. "Uncle Bob" Martin on the *Agile Uprising* podcast**, about the
...
**Draft bio (≈70 words).** *James Gifford builds governance infrastructure for AI-agent software
...
grade its own homework — and lower the grade.*
